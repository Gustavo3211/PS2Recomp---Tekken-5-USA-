...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        {
            const std::filesystem::path cdImage = std::filesystem::absolute(argv[2]);
            if (!std::filesystem::is_regular_file(cdImage))
            {
                throw std::runtime_error("Disc image not found: " + cdImage.string());
            }
            auto ioPaths = PS2Runtime::getIoPaths();
            ioPaths.cdImage = cdImage;
            PS2Runtime::setIoPaths(ioPaths);
            std::cout << "Using disc image: " << cdImage << std::endl;
        }
#if defined(PS2X_ENABLE_DEBUG_UI) && !defined(PLATFORM_VITA)
